...
| 3 | **Rejects (Discard)** | ✅ | Device- + backend-verified: real coldroom-bucket scan → `createDiscardEntry` created a submitted **Discard** Stock Entry **MAT-STE-2026-185754** (Coldroom Bucket - 2879) on live Xflora. Single-scan (`coldroom_bucket` key) + result dialog. Files: `app/(app)/rejects.tsx` + `features/discard/useCreateXfloraDiscardEntry.ts`. Endpoint returns HTTP 200 for both outcomes; branch on `status`/`reason`. No farm gate. Success dialog auto-dismisses (2.5s); too-young → blocking "Too Young" dialog, no entry created. |
